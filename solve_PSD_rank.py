...
def solve(M, print_steps=0, print_rounded=True, eps=0.0000001):
    ...
    print_steps : int
        determines what will be printed, if 0, prints each bound calculated,
        if 1 prints the bounds for psd-rank, if 2 prints only if psd-rank is solved
        (default 0)
    ...
    for func in upper_bound.u_bounds:  # Calculate upper bound using each method
        ...
        ubs.append(res)
    lb_max = max(lbs)
    if math.isclose((lb_max), min(ubs), rel_tol=eps) or math.ceil(lb_max) == min(
        ubs
    ):  # Check if psd-rank was "solved"
        ...
        print(
            f"PSD-rank is within bounds: {math.ceil(lb_max):.1f}<=rank_psd<={min(ubs):.1f}. For matrix: "
